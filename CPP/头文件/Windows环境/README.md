...
**<memoryapi.h>**  
提供内存管理函数：`VirtualAlloc`、`VirtualFree`、`VirtualProtect`、`VirtualQuery` 等，用于操作虚拟内存。  
典型场景：大页内存分配、共享内存、内存映射文件、动态修改内存保护属性。  
现代 C++ 对应：`std::allocator`、`std::pmr::memory_resource` 等，但虚拟内存操作通常需要直接调用系统 API。  
头文件依赖：需包含 `<windows.h>`。

**<errhandlingapi.h>**  
提供错误处理函数：`GetLastError`、`SetLastError`、`RaiseException`、`UnhandledExceptionFilter` 等。  
典型场景：获取系统错误码、结构化异常处理（SEH）、自定义异常过滤。  
现代 C++ 对应：C++ 异常机制（`try`/`catch`）可替代部分场景，但获取 Win32 错误码仍需 `GetLastError`。  
头文件依赖：需包含 `<windows.h>`。
...
**<consoleapi.h>**  
提供控制台操作函数：`AllocConsole`、`FreeConsole`、`SetConsoleTitle`、`ReadConsoleInput` 等。  
典型场景：为 GUI 程序附加控制台窗口、控制台输入输出的高级控制。  
现代 C++ 对应：标准 C++ 的 `<iostream>` 无法直接操作控制台窗口属性，需调用系统 API。  
头文件依赖：需包含 `<windows.h>`。

**<commdlg.h>**  
提供通用对话框函数：`GetOpenFileName`、`GetSaveFileName`、`ChooseFont`、`ChooseColor` 等。  
典型场景：打开/保存文件对话框、字体/颜色选择对话框。  
现代 C++ 对应：无标准库封装，需直接调用系统 API。  
头文件依赖：需包含 `<windows.h>`，并链接 `comdlg32.lib`。
...
**<shlobj.h>**  
提供 Shell 对象和函数：`SHGetFolderPath`、`SHGetKnownFolderPath`、`SHFileOperation`、`IMalloc` 等。  
典型场景：获取特殊文件夹路径（如“我的文档”）、执行高级文件操作（复制/移动/删除）、Shell 编程。  
现代 C++ 对应：C++17 的 `std::filesystem` 可部分替代文件操作，但获取特殊文件夹路径仍需调用系统 API。  
头文件依赖：需包含 `<windows.h>`，并链接 `shell32.lib`。

**<uxtheme.h>**  
提供视觉主题函数：`IsThemeActive`、`OpenThemeData`、`DrawThemeBackground` 等。  
典型场景：检测系统主题是否启用、自定义控件绘制以匹配系统主题样式。  
头文件依赖：需包含 `<windows.h>`，并链接 `uxtheme.lib`。
...
- **定位**：Windows 环境下的头文件提供了对操作系统底层功能的直接访问，包括窗口管理、图形界面、网络通信、文件系统、进程线程、同步机制、内存管理、错误处理、控制台、通用对话框、Shell 编程等。这些接口大多以 C 语言风格暴露，与 Windows 内核紧密耦合。本文档已涵盖最常用的核心功能，对于 DirectX、XInput、音频等专业领域未作展开，可根据需要自行补充。
- **现代 C++ 替代方案**：
