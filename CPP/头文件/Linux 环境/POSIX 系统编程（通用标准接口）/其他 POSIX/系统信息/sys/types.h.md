# `<sys/types.h>` 核心类型与宏详解（Linux / POSIX）

## 说明
`<sys/types.h>` 是 POSIX 标准定义的头文件，它不提供具体的函数，而是定义了系统编程中常用的基本数据类型（如进程 ID、文件偏移量、大小等）。这些类型在文件 I/O、进程控制、网络编程等多个领域广泛使用，通常被其他头文件（如 `<unistd.h>`、`<fcntl.h>`）间接包含。
...
### 1. 类型别名（常见）
以下类型是通过 `typedef` 定义的基本系统数据类型，它们通常对应某些内置类型，但具体大小取决于平台。
