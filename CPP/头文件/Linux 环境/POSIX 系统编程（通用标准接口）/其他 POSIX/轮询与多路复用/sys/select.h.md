# `<sys/select.h>` 核心函数、结构体与宏详解（Linux / POSIX）

## 说明
`<sys/select.h>` 是 POSIX 标准定义的 I/O 多路复用头文件，提供了 `select()` 和 `pselect()` 函数，用于同时监视多个文件描述符的可读、可写或异常事件。本文档按照统一格式整理其中的函数、结构体、宏等信息，并说明内部原理与线程安全性。
...
- `sigmask`：指向信号掩码的指针，在 `pselect` 执行期间会临时替换当前线程的信号掩码，调用结束后恢复。若为 `NULL`，则等同于 `select`。  
**返回值：** 同 `select`。  
**示例用法：**
```c
...
int ret = pselect(sockfd + 1, &readfds, NULL, NULL, &ts, &newmask);
```
**实现原理：**  
...
FD_SET(sockfd, &readfds);    // 添加 sockfd
if (FD_ISSET(sockfd, &readfds)) {  // 检查
    // 就绪
}
FD_CLR(sockfd, &readfds);    // 移除
```
...
- **一级分类：** POSIX 系统编程  
- **二级分类：** 轮询与多路复用  
- **三级分类：** `select`/`pselect` 机制

属于 `POSIX 系统编程 / 轮询与多路复用` 类别。