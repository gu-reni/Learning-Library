## `<netinet/in.h>` 头文件详解（Linux / POSIX）

`<netinet/in.h>` 是 POSIX 标准定义的网络编程头文件，提供了 IPv4 和 IPv6 地址结构、字节序转换函数、地址转换函数以及相关的常量和宏。它是 TCP/IP 网络编程的核心头文件之一，通常与 `<sys/socket.h>` 配合使用。
...
uint32_t addr = htonl(0x0100007F); // 主机 IP 转网络字节序
```

**实现原理：** 这些函数通常被实现为内联函数或宏，在小端机器上执行字节交换，在大端机器上直接返回原值。
...
**线程安全提示：** 线程安全（不依赖静态缓冲区），但返回值 `INADDR_NONE` 与 `255.255.255.255` 的转换结果冲突，建议改用 `inet_aton` 或 `inet_pton`。
