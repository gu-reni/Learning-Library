...
**<sys/uio.h>**  
提供 readv()、writev() 函数，以及 struct iovec 结构，允许在一次系统调用中读写多个非连续缓冲区。  
典型场景：高效处理由多个缓冲区组成的消息（如协议头 + 数据负载），减少系统调用次数。  
现代 C++ 对应：Asio 的 scatter-gather 缓冲区（`buffer` 序列），或使用 `std::vector<iovec>` 配合 `readv`/`writev` 封装。  
头文件依赖：通常独立。
...
**<netinet/tcp.h>**  
定义 TCP 协议相关的选项和结构，如 TCP_NODELAY（禁用 Nagle 算法）、TCP_CORK、TCP_QUICKACK 等。  
典型场景：优化小包延迟（禁用 Nagle）、合并小包发送、控制确认行为。  
现代 C++ 对应：Asio 中可通过 `socket::set_option()` 设置 `ip::tcp::no_delay` 等选项。  
头文件依赖：通常依赖 <netinet/in.h>。
...
**<sys/un.h>**  
定义 struct sockaddr_un 结构，用于 Unix 域套接字（本地进程间通信）。  
典型场景：同一台机器上高效、可靠的进程间通信（IPC），支持流式（SOCK_STREAM）和数据报（SOCK_DGRAM）模式。  
现代 C++ 对应：Asio 中的 `local::stream_protocol` 和 `local::datagram_protocol`。  
头文件依赖：通常独立，需包含 <sys/socket.h>。
...
**<ifaddrs.h>**  
提供 getifaddrs()、freeifaddrs() 函数，用于获取主机上所有网络接口的地址、标志、掩码等信息。  
典型场景：枚举本机 IP 地址（如获取非回环 IPv4 地址）、接口状态检测。  
现代 C++ 对应：无标准库直接对应，可封装系统调用，或使用 Boost.Asio 的 `network_interface` 相关扩展。  
头文件依赖：通常独立。

**<net/if.h>**  
定义网络接口常量（如 IFF_UP、IFF_LOOPBACK）和函数（if_nametoindex、if_indextoname、if_freenameindex 等）。  
典型场景：根据接口名获取索引、根据索引获取接口名、遍历接口列表。  
现代 C++ 对应：Asio 中 `ip::address_v4::from_string` 等不直接提供，需调用系统接口。  
头文件依赖：通常需要 <sys/socket.h>。
