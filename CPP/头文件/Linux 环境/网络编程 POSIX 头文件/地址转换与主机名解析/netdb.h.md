## `<netdb.h>` 头文件详解（Linux / POSIX）

`<netdb.h>` 是 POSIX 标准定义的网络数据库头文件，提供了主机名、服务名、协议名等网络信息的查询接口。它包含主机名解析（`gethostbyname`、`getaddrinfo`）、服务名解析（`getservbyname`、`getservbyport`）等函数，以及相关的结构体和错误码。现代网络编程推荐使用线程安全的 `getaddrinfo` 和 `getnameinfo`，而避免使用旧的 `gethostbyname` 等函数。

---

## 一、函数详解

### 1. gethostbyname（过时，不推荐）
...
**参数：**  
- `name`：主机名（如 `"www.example.com"`）或点分十进制 IPv4 地址。  
...
```
**实现原理：**  
1. 检查输入是否为点分十进制 IPv4 地址，若是则直接构造 `hostent`。  
...
**作用：** 将主机名和服务名转换为套接字地址结构列表（支持 IPv4/IPv6，线程安全）。  
**参数：**  
