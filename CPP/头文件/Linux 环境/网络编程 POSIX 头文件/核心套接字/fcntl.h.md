## `<fcntl.h>` 头文件详解（Linux / POSIX）

`<fcntl.h>` 是 POSIX 标准定义的头文件，提供了文件控制操作和文件描述符管理功能。它包含了 `open`、`creat`、`fcntl` 等函数声明，以及文件打开标志、文件状态标志、文件锁等宏定义。该头文件是进行文件 I/O 系统编程的核心接口之一。
...
- `mode`：当指定 `O_CREAT` 时，用于设置文件权限（如 `0644`），类型为 `mode_t`。
