## `<signal.h>` 头文件详解（Linux / POSIX）

`<signal.h>` 是 C 标准库和 POSIX 标准定义的头文件，提供了信号处理机制。它包含了设置信号处理函数（`signal`、`sigaction`）、发送信号（`raise`、`kill`）、阻塞信号（`sigprocmask`）、等待信号（`sigsuspend`）等函数，以及相关的结构体和宏。信号是软件中断，用于处理异步事件（如用户中断、非法内存访问、定时器到期等）。
...
**实现原理：**
系统调用。内核修改进程的 `sigaction` 表项，记录信号处理方式。当信号递送时，根据表项执行相应操作（调用用户函数、忽略或默认动作）。

**线程安全提示：**
`signal()` 本身是线程安全的，但它的行为在不同系统上可能有差异（如是否自动重置处理函数）。多线程环境中推荐使用 `sigaction` 替代。
...
**实现原理：**
系统调用，内核直接修改进程的 `sigaction` 表项，提供更细粒度的控制（如信号掩码、标志）。

**线程安全提示：**
线程安全。多线程调用时，修改的是进程级别的信号处理方式，所有线程共享。
...
**实现原理：**
系统调用，修改线程的 `sigmask`（信号掩码）。被阻塞的信号不会递送，但会挂起等待。
...
sigset_t newmask;
sigemptyset(&newmask);
sigsuspend(&newmask);   // 等待任何信号，除了已阻塞的
```

**实现原理：**
系统调用，原子地设置掩码并进入可中断睡眠，信号递送后恢复原掩码。
...
**实现原理：**
系统调用，将线程挂起直到信号集中的任一信号递送。信号不经过用户处理函数。
...
---
（注：它也是 C 标准库的一部分，属于 `标准库 / C 标准库 / 信号处理`，但按 POSIX 系统编程分类更符合网络编程上下文。）