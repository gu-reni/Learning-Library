...
并发读写触发panic的核心机制分为以下几步：
 - **写操作开始时**，运行时会将`flags`字段的`hashWriting`位设置为1，标记当前有写操作正在进行
 - **读操作或其他写操作发起时**，运行时会检查`hashWriting`标志位是否被置位
 - **一旦检测到冲突**，运行时直接调用`throw`函数，触发不可恢复的`fatal error`，即panic
 - **写操作完成后**，运行时清除`hashWriting`标志位，恢复正常状态
