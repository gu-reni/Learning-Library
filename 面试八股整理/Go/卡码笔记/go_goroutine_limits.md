...
A3：优雅关闭 goroutine 通常需要传递**退出信号**，主要方式有：
 - **使用 `context` 标准库**（推荐）：通过 `context.WithCancel`、`WithTimeout` 等衍生出 ctx，在 goroutine 内部监听 `ctx.Done()`，一旦接收到取消信号就执行清理工作并退出。
 - **使用 `channel`**：专门创建一个 `done` 通道，当需要关闭 goroutine 时关闭该通道（`close(done)`），goroutine 内部通过 `select` 监听该通道实现优雅退出。