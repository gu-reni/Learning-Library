...
A2：Go 的 goroutine 调度策略主要包括：
 - **抢占式调度**：基于信号的异步抢占机制，避免某个 goroutine 长时间占用 CPU；
 - **work-stealing（工作窃取）机制**：当某个 P 的本地队列为空时，会从其他 P 的队列或全局队列中“窃取” G 来执行，以此平衡各个 P 的负载；
 - **系统调用处理（Handoff 机制）**：当 goroutine 进行阻塞式系统调用时，执行该 G 的 M 会与当前的 P 解绑，P 会寻找或创建新的 M 来继续调度执行队列中的其他 goroutine。这些策略共同保证了 Go 程序的高效并发执行。
