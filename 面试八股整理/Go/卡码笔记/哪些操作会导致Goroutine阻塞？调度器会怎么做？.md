...
当 Goroutine 阻塞时，Go 调度器的处理方式取决于阻塞的类型：
 - **仅阻塞 G，不阻塞 M**：调度器会将该 Goroutine 挂起，把它从当前 M 上移除，然后让当前 M 继续从 P 的本地队列或全局队列中获取其他就绪的 Goroutine 执行。
 - **同时阻塞 G 和 M：Go 的系统监控会介入，将当前的 P 与被阻塞的 M 分离，并把 P 交给其他空闲的 M 或新建一个 M 去继续执行队列中的其他任务，以此保证 CPU 资源不被浪费。
