...
一共四步：
 - 算这个向量的均值 μ\muμ
 - 算这个向量的标准差 σ\sigmaσ
 - 用 (x−μ)/σ(x - \mu) / \sigma(x−μ)/σ 标准化，每个维度都"缩"到均值 0、方差 1
 - 再乘 γ\gammaγ（缩放）、加 β\betaβ（偏移）——这两个是可学习参数，让模型自己决定"归一化到什么程度"
...
### # 步骤汇总
 组件 公式 核心作用 残差连接 F(x)+xF(x) + xF(x)+x 给梯度留高速公路，防止深层网络训练崩 LayerNorm x−μσ⋅γ+β\frac{x-\mu}{\sigma} \cdot \gamma + \betaσx−μ​⋅γ+β 把每个 Token 的向量拉回均值 0、方差 1 Add & Norm LayerNorm(x+F(x))\text{LayerNorm}(x + F(x))LayerNorm(x+F(x)) 两者捆绑，每个子层后面都挂一个 
---
