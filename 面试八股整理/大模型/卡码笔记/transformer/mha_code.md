...
### # 五步流程
 步骤 操作 输入形状 输出形状 ① 线性映射 X 乘 WQ/WK/WVW_Q / W_K / W_VWQ​/WK​/WV​ (L,dmodel)(L, d_model)(L,dm​odel) (L,dmodel)(L, d_model)(L,dm​odel) ② Head 拆分 reshape + transpose $ (L, d_model) $ (h,L,dk)(h, L, d_k)(h,L,dk​) ③ 并行 Attention 每个头独立跑完整 Attention (L,dk)×h(L, d_k) × h(L,dk​)×h (L,dk)×h(L, d_k) × h(L,dk​)×h ④ Concat 拼接 transpose + reshape (h,L,dk)(h, L, d_k)(h,L,dk​) (L,dmodel)(L, d_model)(L,dm​odel) ⑤ 输出投影 乘 WOW_OWO​ (L,dmodel)L, d_model)L,dm​odel) (L,dmodel)(L, d_model)(L,dm​odel) 
