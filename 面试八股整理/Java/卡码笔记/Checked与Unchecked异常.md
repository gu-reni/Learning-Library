...
#### Checked Exception 和 Unchecked Exception 的概念
 - **Checked Exception (受检查异常，也叫编译期异常)** ：是指所有**直接或间接**继承自 `java.lang.Exception`，但不是属于 `java.lang.RuntimeException` 及其子类的异常。编译器会强制检查这类异常。
 - **Unchecked Exception (非受检查异常，也叫运行期异常)** ：是指属于 `java.lang.RuntimeException` 类及其所有子类。编译器不强制检查这类异常。
...
### 知识拓展
 ...
 - **面试官可能的追问1：在什么情况下，我可以将一个编译期异常包装成运行期异常抛出？这样做有什么优缺点？** **简答：**
...
④ **良好实践**：如果选择包装，可以将原始异常作为新异常的 `cause`（原因）链起来，例如 `throw new RuntimeException("Error occurred", FileNotFoundException);`。
 - **面试官可能的追问2：为什么 Java 强制检查 Checked Exception，而 C++ 或 Python 等语言没有类似机制？这种设计哲学有何不同？** **简答：**
