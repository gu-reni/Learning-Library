...
### # 知识拓展
 - **`Collections.synchronizedMap(Map<K,V> m)`：** 这是一个工具方法，可以将任何非线程安全的 `Map`（如 `HashMap`、`LinkedHashMap`、`TreeMap`）包装成一个线程安全的 `Map`。
 - 它的实现机制是**对传入的 `Map` 对象进行包装，并在每个方法调用时对整个 `Map` 对象加锁**。虽然能提供线程安全，但由于是粗粒度的锁，其并发性能通常不如 `ConcurrentHashMap`。在并发度要求不高的场景下可以使用。
 - **`WeakHashMap`：** 其键是弱引用，当键不再被其他强引用指向时，即使 `WeakHashMap` 中还存有它的引用，垃圾回收器也会回收这个键值对。
