...
### 简要回答
 - **使用"=="进行比较**：
==是一个**比较运算符**，既可以判断基本类型，又可以判断引用类型。
 - 如果**判断基本类型**，判断的是二者的**值**是否相等(eg : 判断1 == 1，结果为true；判断1 == 3，结果为false)；
 - 如果**判断引用类型**，判断的是二者的**地址**是否相同，即判定是否为同一对象(eg : Student student1 = new Student();，Student student2 = student1；判断student2 == student1，结果为true)。
 - **使用equals()方法进行比较**：
