...
- **MySQL C API**：`<mysql/mysql.h>` 提供了与 MySQL 服务器通信的函数和类型。
- 标准 C 库：`stdio.h`、`stdlib.h`、`string.h` 用于文件操作、内存管理和字符串处理。

### 2.2 关键数据类型

- **`MYSQL`**：MySQL 连接句柄，代表一个数据库连接。
- **`MYSQL_STMT`**：预处理语句句柄，用于执行参数化查询。
- **`MYSQL_BIND`**：绑定参数和结果的结构体，用于在 C 变量和预处理语句之间传递数据。
- **`MYSQL_RES`**：结果集元数据，用于获取字段信息。
- **`MYSQL_FIELD`**：字段信息结构体，包含字段名、类型、长度等。

## 3. 主要函数思路详解

### 3.1 文件读取函数 `read_file`

```c
unsigned char* read_file(const char *filename, size_t *size)
```

**设计思路**：将图片文件以二进制方式读入内存，返回缓冲区指针，同时通过 `size` 参数返回文件大小。该函数用于准备插入到数据库的 BLOB 数据。

- 以 `"rb"` 模式打开文件，确保二进制读取。
- 使用 `fseek` 和 `ftell` 获取文件大小。
- 动态分配足够大的缓冲区，读取整个文件内容。
- 关闭文件，返回缓冲区指针（调用者负责释放）。

### 3.2 插入图片 `insert_image`

**设计思路**：通过预处理语句向 `images` 表插入一条记录，包含图片名称和图片二进制数据。

- 提示用户输入文件路径和图片名称。
- 调用 `read_file` 读取图片数据到内存。
- 初始化预处理语句，准备 SQL：`INSERT INTO images (name, image_data) VALUES (?, ?)`。
- 绑定两个参数：
  - 第一个为字符串类型（`MYSQL_TYPE_STRING`），指向图片名称，长度由 `buffer_length` 指定。
  - 第二个为 BLOB 类型（`MYSQL_TYPE_BLOB`），指向图片数据缓冲区，长度由 `buffer_length` 指定实际数据大小。
- 执行语句，检查影响行数，输出结果。
- 释放预处理语句和图片数据缓冲区。
...
- 提示用户输入图片 ID 和保存路径。
- 准备查询 SQL：`SELECT image_data FROM images WHERE id = ?`。
- 绑定输入参数（ID）为 `MYSQL_TYPE_LONG`。
- 执行查询，调用 `mysql_stmt_store_result` 缓存结果集。
- 获取结果集元数据，确定 BLOB 字段的最大可能长度（但实际长度需从 fetch 获取）。
- 绑定结果集：先绑定一个空缓冲区，执行 `mysql_stmt_fetch` 获取数据长度（`data_len`）。
- 根据实际长度分配缓冲区，重新绑定结果集，再次 `mysql_stmt_fetch` 获取真实数据。
- 将数据写入本地文件。
- 释放相关资源（元数据、结果集、预处理语句、数据缓冲区）。

### 3.4 创建 `images` 表 `create_images_table`

```c
void create_images_table(MYSQL *conn)
```

- 执行 SQL：`CREATE TABLE IF NOT EXISTS images (...)`
- 表结构包含自增主键 `id`、图片名称 `name`、图片数据 `image_data`（`LONGBLOB`）、上传时间 `upload_time`。

### 3.5 插入用户数据 `insert_user`

**设计思路**：向 `tbl_user` 表插入多条用户记录，使用预处理语句和循环输入，直到用户输入 `quit` 退出。

- 准备 SQL：`INSERT INTO tbl_user (U_NAME, U_GENDER) VALUES (?, ?)`。
- 绑定两个字符串参数，并指定长度指针（`name_len`, `gender_len`），以支持不同长度的输入。
- 循环提示用户输入姓名和性别，每次输入后执行语句。
- 当输入 `quit` 时退出循环。
- 关闭预处理语句。
...
- 连接数据库（`mysql_real_connect`），指定主机、用户名、密码、数据库名。
- 设置字符集为 `utf8mb4` 以支持中文等字符。
- 调用 `create_images_table` 确保表存在。
- 进入菜单循环，处理用户选择：1 插入用户，2 插入图片，3 读取图片，4 退出。
- 关闭连接。
...
void insert_image(MYSQL *conn) {
    ...
    bind[0].length = NULL;      // 字符串类型由 buffer_length 决定长度，无需额外 length

    // 第二个参数：图片数据（二进制 BLOB）
    bind[1].buffer_type = MYSQL_TYPE_BLOB;
    bind[1].buffer = file_data;
    bind[1].buffer_length = file_size;   // 实际数据大小
    bind[1].length = NULL;
...
void read_image(MYSQL *conn) {
    ...
    // 获取结果集元数据，主要用于获取字段信息（此处用于调试，实际不需要）
    ...
    // 此处可以不使用 field_len，仅作为示例
    // MYSQL_FIELD *field = mysql_fetch_field(res_meta);
    // unsigned long field_len = field->length; // 字段定义的最大长度

    // 绑定结果集：先绑定一个空缓冲区以获取实际数据长度
    MYSQL_BIND result_bind;
    unsigned long data_len = 0;
    memset(&result_bind, 0, sizeof(result_bind));
    result_bind.buffer_type = MYSQL_TYPE_BLOB;
    result_bind.buffer = NULL;          // 先不分配缓冲区
    result_bind.buffer_length = 0;
    result_bind.length = &data_len;      // 输出参数，存储实际数据长度
    ...
    // 第一次 fetch：获取数据长度（此时 data_len 被填充）
...
- 分块读取和插入：使用 `LONG BLOB` 流式插入，例如使用 `mysql_stmt_send_long_data` 分块发送数据。
...
**答**：代码假设 `tbl_user` 已存在，未做检查。实际应用中应添加创建表的逻辑，或先查询表是否存在。示例中仅用于演示预处理语句的循环插入，生产环境需完善。
